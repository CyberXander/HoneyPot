...
class SshSocketServer extends SocketServer {
	start() {
			...
			client.on('authentication', (ctx) => {
				if (ctx.method !== 'password') return ctx.reject(['password']);
					...
					if (client._client_info) {
						...
						if (!failedLoginAttempts[ip]) failedLoginAttempts[ip] = 0;

						if (checkBruteForce(failedLoginAttempts, ip)) {
							this.emit('bruteforce', {
								'ip': ip,
								'username': username,
								'password': password,
								'service': this.name,
								'request': (username && username.length !== '') ? this.name + ' ' + username + '@' + config.server_ip + ':' + this.port : this.name + ' ' + config.server_ip + ':' + this.port,
		...
		function checkBruteForce(failedLoginAttempts, ip) {
			const failedAttempts = failedLoginAttempts[ip] || 0;
